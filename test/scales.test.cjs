...
vows.describe('Some tests for scale()')
    ...
    .addBatch({
        ...
        'get colors from a scale with more than two colors': {
            ...
            'just origianl colors'(topic) {
                assert.deepEqual(topic.f.colors(), [
                    '#ffff00',
                    '#ffa500',
                    '#006400'
                ]);
        ...
        'weird result': {
            ...
            'has hex function at 0.5'(topic) {
                assert.equal(typeof topic.f(0.5).hex, 'function');
            },
            'has hex function at 0'(topic) {
        ...
        'scale with one color': {
            ...
            'should return that color'(topic) {
        ...
        'scale() no data color': {
            ...
            'undefined --> nodata'(topic) {
                assert.equal(topic.f(undefined).hex(), '#cccccc');
            },
            'custom nodata color'(topic) {
                assert.equal(
                    topic.f.nodata('#eee')(undefined).hex(),
                    '#eeeeee'
                );
            }
        },
