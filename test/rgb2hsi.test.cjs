...
Object.keys(tests).forEach((key) => {
    batch[`rgb2hsi ${key}`] = {
        ...
        args(topic) {
            ...
            assert.deepStrictEqual(
                rgb2hsi.apply(null, topic.rgb).map(rnd),
                topic.hsi.map(rnd)
            );
        }
    };
...
vows.describe('Test rgb2hsi color conversions').addBatch(batch).export(module);
