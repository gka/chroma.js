...
vows.describe('Some tests for chroma.limits()')

    .addBatch({
        'simple continuous domain': {
            topic: limits([1, 2, 3, 4, 5], 'continuous'),
            domain(topic) {
                return assert.deepEqual(topic, [1, 5]);
            }
        },
        ...
        'continuous domain, negative values': {
            topic: limits([1, -2, -3, 4, 5], 'continuous'),
            domain(topic) {
                return assert.deepEqual(topic, [-3, 5]);
            }
        },
        ...
        'equidistant domain': {
            ...
            domain(topic) {
                return assert.deepEqual(topic, [0, 2, 4, 6, 8, 10]);
            }
        },

        'equidistant domain, NaN values': {
            topic: limits(
                [0, 9, 3, 6, 3, 5, undefined, Number.NaN, 10],
                'equidistant',
                5
            ),
            domain(topic) {
        ...
        'logarithmic domain': {
            ...
            domain(topic) {
                return assert.deepEqual(topic, [1, 10, 100, 1000, 10000]);
            }
        },
        ...
        'quantiles domain': {
            ...
            domain(topic) {
                return assert.deepEqual(topic, [1, 4.5, 100]);
            }
        },
        ...
        'quantiles not enough values': {
            ...
            domain(topic) {
                return assert.deepEqual(topic, [0, 0.2, 0.4, 0.6, 0.8, 1]);
            }
        }
    })
